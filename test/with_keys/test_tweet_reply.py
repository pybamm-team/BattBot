...
class TestTweetReply(unittest.TestCase):
    def test_tweet_reply(self):
        ...
        with pytest.raises(
            Exception,
            match="Please provide at least 2 models. Some tweet examples -"
            f" {request_examples}",
        ):
            reply.generate_reply(tweet_text, testing=True)
        ...
        with pytest.raises(
            Exception,
            match="I'm sorry, I couldn't understand the requested simulation. "
        ...
        with pytest.raises(
            Exception,
            match="Please provide a parameter set in the format - Chen2020. "
            + f"Some tweet examples - {request_examples}",
        ):
            reply.generate_reply(tweet_text, testing=True)
        ...
        with pytest.raises(
            Exception,
            match="Please provide 'Ambient temperature' in the format - "
        ...
        with pytest.raises(
            Exception,
            match="Please provide 'C rate' in the format - "
